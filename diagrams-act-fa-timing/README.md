...
- Линия производства обнаруживает дефект и сообщает в dispatch_ui.
- dispatch_ui запрашивает у defect_system свободные зоны и отправляет машину в подходящую зону.
- defect_system регистрирует дефект; worker_ui обновляет доступность.
- defect_system уведомляет foreman_ui, тот назначает работника.
- defect_system выдаёт задачу в worker_ui; работник начинает и завершает ремонт.
- Система сообщает в quality_service о готовности, получает результат теста.
- Если тест пройден — возврат на производство; если нет — повторный дефект‑флоу через dispatch_ui.
